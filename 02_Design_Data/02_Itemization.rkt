...
;; Countdown -> Image
;; products nice image of countdown current state
(check-expect (countdown-to-image false)(square 0 "solid" "white"))
(check-expect (countdown-to-image 1)(text (number->string 1) 15 "olive"))
(check-expect (countdown-to-image "complete")(text "Happy new Year!" 15 "red"))
...
(define (countdown-to-image c)
  (cond [(false? c)
        ...
        [(and (number? c)
              (<= 1 c)
              (<= c 10))
         (text (number->string c) 15 "olive")]
        [else
         (text "Happy new Year!" 15 "red")]))
